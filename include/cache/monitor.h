...
static inline void pprint_cache_acc_recs(cache_acc_rec* recs, size_t n_recs) {
	for (size_t n = 0; n < n_recs; n++) {
		printf(
			"Recv %2lu: tsc: %lu; aux: %u; iters: %lu; lat: %u; "
			"blind: %u\n",
...
static __always_inline i64 probe_skx_sf_evset_para_asm(EVSet* evset,
													   u64* end_tsc,
													   u32* aux) {
...
static __always_inline i64 probe_skx_sf_evset_para_noasm(EVSet* evset,
														 u64* end_tsc,
														 u32* aux) {
	u8** addrs = evset->addrs;
	_force_addr_calc(addrs);
...
static __always_inline i64 probe_skx_sf_evset_ptr_chase(EVSet* evset,
														u64* end_tsc,
														u32* aux) {
...
static __always_inline i64 probe_skx_sf_evset_para(EVSet* evset,
												   u64* end_tsc,
												   u32* aux) {
...
void prime_skx_sf_evset_ps_sense(evchain* chain1,
								 evchain* chain2,
								 bool prime_sense,
								 EVSet* lower);

void prime_skx_sf_evset_ps_flush(EVSet* evset,
								 evchain* chain,
								 u32 arr_repeat,
								 u32 l2_repeat);

i64 calibrate_para_probe_lat(u8* target,
							 EVSet* evset,
							 u32 arr_repeat,
							 u32 l2_repeat,
							 double bad_thresh_ratio);

i64 calibrate_chase_probe_lat(u8* target,
							  EVSet* evset,
							  u32 arr_repeat,
							  u32 l2_repeat,
							  double bad_thresh_ratio);
