...
i64 evset_test_batch(u8 **targets, size_t cnt, EVSet *evset)
{
	...
	if (batch_sz > 2)
		batch_sz -= 1; // batch_sz = max(n_ways - 1, 1);
...
static inline void helper_thread_traverse_cands(u8 **cands,
												size_t cnt,
												EVTestConfig *conf)
{
...
EVTestRes generic_test_eviction(u8 *target,
								u8 **cands,
								size_t cnt,
								EVTestConfig *tconf)
{
...
bool evset_builder_last_straw(u8 *target, EVSet *evset)
{
	...
	while (evsz < evset->cap && n_bctr < max_bctr)
	{
		...
		_dprintf(
			"Final %ld: Upper: %lu; Lower: %lu; has_pos: %d; has_neg: %d; "
			"offset: %u; evsz: %lu\n",
...
{
	config->cands_config = (EVCandsConfig){.scaling = 1, .filter_ev = NULL};
	config->test_config =
...
{
	config->cands_config = (EVCandsConfig){.scaling = 3, .filter_ev = NULL};
	config->test_config =
...
void default_skx_sf_evset_build_config(EVBuildConfig *config,
									   EVSet *l1d_ev,
									   EVSet *l2_ev,
									   helper_thread_ctrl *hctrl)
{
	config->cands_config = (EVCandsConfig){.scaling = 3, .filter_ev = l2_ev};
	config->test_config =
...
EVSet *build_evset_generic(u8 *target,
						   EVBuildConfig *config,
						   cache_param *cache,
						   EVCands *evcands)
{
	...
	if (!can_evict && !config->algo_config.ret_partial)
		goto err;
...
EVSet **build_evsets_at(u32 offset,
						EVBuildConfig *conf,
						cache_param *cache,
						EVCands *_cands,
						size_t *ev_cnt,
	...
	l2_ev_fail:
		...
		if (addrs)
		{
			...
			if (!found)
			{
				...
				_error(
					"Cannot find the next target: cands size: %lu; addr: %lu\n",
					cands->size, acc_cnt);
